...
const createStyles = (C: typeof Colors) => StyleSheet.create({
  ...
  periodeRow: { flexDirection: 'row', padding: 16, gap: 8, paddingTop: 24 },
  periodePill: {
    flex: 1, paddingVertical: 8, borderRadius: 20,
    borderWidth: 1.5, borderColor: C.greyBorder, alignItems: 'center',
    backgroundColor: C.surface,
  },
  periodePillActive: { backgroundColor: C.primaryLight, borderColor: Colors.primary },
  periodeText: { fontSize: 12, color: C.grey, fontWeight: '600' },
  periodeTextActive: { color: Colors.primary },
  wilayaBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    marginHorizontal: 16, marginBottom: 10, paddingHorizontal: 14, paddingVertical: 9,
    borderRadius: 10, borderWidth: 1.5, borderColor: C.greyBorder, backgroundColor: C.surface,
  },
  ...
  myRankItem: { backgroundColor: C.primaryLight, borderColor: Colors.primary },
  rankPos: { width: 28, fontSize: 14, fontWeight: '700', color: C.grey, textAlign: 'center' },
  avatar: {
  ...
  rankName: { fontSize: 14, fontWeight: '700', color: C.primaryDark },
  rankSub: { fontSize: 12, color: C.grey, marginTop: 2 },
  rankPts: { fontSize: 14, fontWeight: '700', color: C.grey },
...
export default function ClassementScreen() {
  ...
  const PERIODES = useMemo(() => [
    { label: t('ranking.thisWeek'), value: 'week' },
    { label: t('ranking.thisMonth'), value: 'month' },
    { label: t('ranking.global'), value: 'global' },
  ], [t]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={
            myRank && myRank.rang > 10 ? (
              <View style={[styles.rankItem, styles.myRankItem]}>
                ...
                <RankAvatar item={myRank} isMe styles={styles} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.rankName}>{myRank.prenom} {myRank.nom}</Text>
                  <Text style={styles.rankSub}>Niv. {myRank.niveau} · {myRank.wilaya || t('ranking.algeria')}</Text>
                </View>
          ...
          renderItem={({ item, index }) => {
            ...
            return (
              <View style={[styles.rankItem, isMe && styles.myRankItem]}>
                ...
                <RankAvatar item={item} isMe={isMe} styles={styles} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.rankName}>{item.prenom} {item.nom}</Text>
                  <Text style={styles.rankSub}>Niv. {item.niveau} · {item.wilaya || t('ranking.algeria')}</Text>
                </View>
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Ionicons name="trophy-outline" size={48} color={C.greyBorder} />
              <Text style={styles.emptyText}>
                {wilaya ? t('ranking.noResultForWilaya', { wilaya }) : t('ranking.noRanking')}
              </Text>
            </View>
          }
        />
...
function RankAvatar({ item, isMe, styles }: { item: RankUser; isMe: boolean; styles: ReturnType<typeof createStyles> }) {
